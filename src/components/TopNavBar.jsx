...
const Breadcrumb = ({ items }) => {
  ...
  const [isMute,setIsMute] = useState(localStorage.getItem('mute'));
            ...
            <button className="bg-purple-100 p-2 rounded-full text-twilight-500 hover:bg-purple-200 focus:outline-none"
                onClick={() => {handleSoundClick()}}>
                {
                  <div className="flex items-center ">
                    {isMute === 'true' ? <Volume size={20} /> : <Volume2 size={20} />}
                    {isMute === 'true' ? <p className="text-base text-twilight-400"> Unmute {botName} </p> : <p className="text-base text-twilight-400 ml-1">Mute {botName} </p>}
                  </div>
